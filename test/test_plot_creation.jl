...
function test_plots(file_path::String; backend_pkg::String = "gr")
    ...
    @testset "test $backend_pkg plot production" begin
        ...
        plot_dataframe(gen_uc.data[:P__ThermalStandard], gen_uc.time, display = display, title = "df_line", save = out_path )
        plot_dataframe(gen_uc.data[:P__ThermalStandard], gen_uc.time, display = display, title = "df_stack", save = out_path, stack = true)
        plot_dataframe(gen_uc.data[:P__ThermalStandard], gen_uc.time, display = display, title = "df_stair", save = out_path, stair = true)

        plot_dataframe(gen_uc.data[:P__ThermalStandard], gen_uc.time, display = display, title = "df_bar", save = out_path, bar = true)
        plot_dataframe(gen_uc.data[:P__ThermalStandard], gen_uc.time, display = display, title = "df_bar_stack", save = out_path, bar = true, stack = true)

        p = plot_dataframe(gen_uc.data[:P__ThermalStandard], gen_uc.time, display = false, stack = true);
        plot_dataframe(p, load_uc.data[:P__PowerLoad] .* -1, gen_uc.time, display = display, title = "df_gen_load", save = out_path )
